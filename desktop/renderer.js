...
function update(value){const wasPaused=state.paused;state=value;$('connection').textContent=value.connected?'사이트 연결됨':'브라우저 연결 필요';$('pause').textContent=value.paused?'안내 재개':'안내 멈춤';if(value.paused&&!wasPaused)stop();}
...
$('site').onclick=()=>window.jarvis.site();$('extension').onclick=()=>window.jarvis.extension();$('pair').onclick=async()=>{await window.jarvis.copyPairing();$('status').textContent='연결 코드를 복사했어요. 브라우저의 자비스 확장에 붙여넣으세요.';};
